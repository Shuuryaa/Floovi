  ...
  client.on("messageCreate", async (message) => {
          ...
          if (commandCount > 5) {
            ...
            if (!checkisBlacklisted) {
              ...
              const saixd = new EmbedBuilder()
                ...
                .addFields({ name: 'Support Server', value: '[Join our support server](https://discord.gg/G4Uc7mwfwM)', inline: true })
                .setTimestamp();
    ...
    if (
      ...
      const embed = new EmbedBuilder()
        .setDescription(`<:cross:1341048111740489851> | You don't have enough Permissions !!`)
    ...
    if (
      ...
      const embed = new EmbedBuilder()
        .setDescription(`<:cross:1341048111740489851> | I don't have enough Permissions !!`)
        .setColor(client.config.color);
      return message.reply({ embeds: [embed] });
    }
    if (command.inVc && !message.member.voice.channel) {
      const embed = new EmbedBuilder()
        .setDescription(
          "<:cross:1341048111740489851> | You need to be in a voice channel to use this command!"
        )
        .setColor(client.config.color);
      return message.reply({ embeds: [embed] });
    }
    if (
      ...
      const embed = new EmbedBuilder()
        .setColor(client.config.color)
        .setDescription(`<:cross:1341048111740489851> | I'm not in the same voice channel as you!`);
      return message.reply({ embeds: [embed] });
    }
    ...
    if (command.voteOnly && client.config.vote) {
      ...
      if (!vote) {
        ...
        const embed = new EmbedBuilder()
          ...
          .setDescription(
            `Hello!
Thank you for showing interest in using the ${command.name} command. This particular feature is part of the commands that are accessible after supporting our bot by voting on Top.gg.
...
Thank you once again for your support and understanding! 💖`
    ...
    if (command.ownerOnly) {
      ...
      if (!owner) {
        const embed = new EmbedBuilder()
          .setColor(client.color)
          .setDescription(
            "<:cross:1341048111740489851> | It's a owner only command you can't use this command!"
          );
      ...
      try {
        if (!role) {
          const embed = new EmbedBuilder()
            .setColor(client.color)
            .setDescription(
              "<:cross:1341048111740489851> | The DJ role could not be found. It may have been deleted. Please check the server settings."
            );
          return message.channel.send({ embeds: [embed] });
        }
        ...
        if (!member.roles.cache.has(role.id)) {
          const embed = new EmbedBuilder()
            .setColor(client.color)
            .setDescription(
              "<:cross:1341048111740489851> | You do not have the required DJ role to use this command."
            );
          return message.channel.send({ embeds: [embed] });
        }
      } catch (error) {
    ...
    if (restrictionData) {
      if (restrictionData.restrictedTextChannels.includes(message.channel.id)) {
        const embed = new EmbedBuilder()
          .setColor(client.config.color)
          .setDescription("<:floovi_warn:1382779289858211880> | This text channel is restricted and you cannot use commands here!");
      ...
      if (message.member.voice.channelId && restrictionData.restrictedVoiceChannels.includes(message.member.voice.channelId)) {
        const embed = new EmbedBuilder()
          .setColor(client.config.color)
          .setDescription("<:floovi_warn:1382779289858211880> | You are in a restricted voice channel and cannot use commands!");
    ...
    if (isChannelIgnored) {
      const embed = new EmbedBuilder()
        .setColor(client.config.color)
        .setDescription("<:floovi_warn:1382779289858211880> | This Channel is in my Ignore List, You can't use my commands here!");
